from psychopy import core, visual, gui, data, event, monitors, sound, iohub
...
winsound=sound.Sound('C:\\Users\\acpike\\Documents\\voltrain\\stimuli\\cha_ching.wav')   # sound to play on win
losssound=sound.Sound('C:\\Users\\acpike\\Documents\\voltrain\\stimuli\\error2.wav')     # sound to play on loss
minjit=2        # min jitter duration in secs
maxjit=6        # max jitter duration in secs
stimlets=['a','F','G','h','K','o']      # letters to be used as stimuli

main_dir='C:\\Users\\acpike\\Documents\\voltrain'
os.chdir(main_dir)
...
trialdat=['Trialnumber','Winpos','Losspos','Side','Order','Choice','RT','Choiceside','Winchosen','LossChosen','TotalMoney','Fixonset','Choiceonset','Monitoronset','Winresonset','Lossresonset','stima','stimb']


# write basic info, task parameters and headers
writeToFile(dataFile,['Subject Num',expInfo['Subject Num'],'Traintype',expInfo['Traintype'],'date',expInfo['dateStr']])
...
#for indtrial in trialhandle:
    ...
    fixonset,choiceonset,monitoronset,winresonset,lossresonset,stima.text,stimb.text]))
    
    ntrial+=1
    
    if ntrial == 60 | ntrial ==120:
...
tracker.record_off()
# Retrieve EDF
tracker.end_experiment('C:\\Users\\acpike\\Documents\\voltrain\\results\\tracker')
os.rename(os.path.join('C:\\Users\\acpike\\Documents\\voltrain\\results\\tracker\\',trackname+'.edf'),os.path.join('C:\\Users\\acpike\\Documents\\voltrain\\results\\tracker\\',fileName+'_tracker.edf'))
